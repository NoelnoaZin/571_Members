    ...
    <nav class="nav">
      <div class="nav-inner">
        ...
        <div class="music glass">
          <button id="musicBtn" aria-label="Toggle music">${ICONS.mute}</button>
          <input id="musicVol" type="range" min="0" max="1" step="0.01" value="0.4" aria-label="Volume"/>
        </div>
      </div>
    </nav>
  `;
...
function initNavbar(active){
  ...
  btn.addEventListener("click", async () => {
    if(!_audio){ _audio = new Audio(TRACK_URL); _audio.loop = true; _audio.volume = parseFloat(vol.value); }
    if(playing){ _audio.pause(); playing = false; btn.innerHTML = ICONS.mute; }
    else { try{ await _audio.play(); playing = true; btn.innerHTML = ICONS.vol; }catch(e){} }
  });
  vol.addEventListener("input", () => { if(_audio) _audio.volume = parseFloat(vol.value); });
